import {userLoginT, userT} from "../types";
...
loginRouter.post('/', async (req: Request, res: Response) => {
    ...
    if(loginOrEmail.includes('@')){
        findUser = await patreonUsers.find({email: loginOrEmail}).toArray()
    } else {
        findUser = await patreonUsers.find({username: loginOrEmail}).toArray()
    }
